...
if (-Not ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] 'Administrator')) {
    if ([int](Get-CimInstance -Class Win32_OperatingSystem | Select-Object -ExpandProperty BuildNumber) -ge 6000) {
        Write-Host "The script will ask for admin privileges. Do you want to continue? [Y/N]"
...
Clear-Host
Write-Host "======================================="
Write-Host "= Removal script for yoursearchbar.me ="
Write-Host "======================================="
...
if ($infectedScripts.Count -gt 0) {
    ...
    Write-Host "`nDo you want to remove them? [Y/N]"
    ...
    Write-Host "Infected scripts removed."
} else {
    Write-Host "No infected scripts found."
}
...
Write-Host "`nSearching for infected shortcuts..."
$shortcuts = Get-ChildItem -Path "C:\" -Include *.lnk -Recurse -Force -ErrorAction SilentlyContinue
$shell = New-Object -ComObject WScript.Shell

Write-Host "Removing malicious arguments on shortcuts:"
...
if (Test-Path $internalKernelGrid4Path) {
    Write-Host "`nFound malicious folder '$internalKernelGrid4Path'!"
    Remove-Item -Path "$internalKernelGrid4Path\*" -Recurse -Force
    Remove-Item -Path $internalKernelGrid4Path -Force
    Write-Host "Folder '$internalKernelGrid4Path' has been deleted."
}
...
if (Test-Path -Path $extensionFolderPath) {
    Write-Host "`nFound malicious extension folder '$extensionID' in '$chromeExtensionsPath'!"
    Remove-Item -Path $extensionFolderPath -Recurse -Force
    Write-Host "Extension folder '$extensionID' has been deleted from '$chromeExtensionsPath'"
}
...
if ($corruptedDlls.Count -gt 0)
{
    ...
    Write-Host "The following files are potentially corrupted:"
    $corruptedDlls | ForEach-Object { Write-Host $_.Fullname }

    Write-Host "`nDo you want to try replacing them with a non corrupted DLL? This could break Microsoft Edge but it is required to remove the malware. [Y/N]"
    if ((Read-Host).ToUpper() -ne "Y") {
        Write-Host "Aborting..."
        exit
    }
